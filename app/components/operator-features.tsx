...
export function AssignmentsConsole() {
  ...
  return <ConsoleShell consoleType="admin">
    ...
    <div className="metric-grid four live-metrics"><Metric label="Upcoming Trips" value={String(trips.length)} note="Ready for assignment" /><Metric label="Active Vehicles" value={String(vehicles.length)} note="Capacity-safe fleet" /><Metric label="Active Drivers" value={String(drivers.length)} note="Licensed drivers" /><Metric label="Assigned Trips" value={String(assignments.length)} note="Active assignments" /></div>
...
export function RefundConsole({ consoleType }: { consoleType: "admin" | "superadmin" }) {
  ...
  return <ConsoleShell consoleType={consoleType}>
    ...
    <div className="metric-grid four live-metrics"><Metric label="Refund Requests" value={String(refunds.length)} note="Total refund requests" /><Metric label="Eligible Bookings" value={String(bookings.length)} note="Paid or partial bookings" /><Metric label="Pending Review" value={String(refunds.filter((r) => r.status === "requested").length)} note="Awaiting operator review" /><Metric label="Total Refunded" value={`₱${refunds.filter((r) => r.status === "succeeded").reduce((acc, r) => acc + Number(r.amount), 0).toLocaleString("en-PH")}`} note="Simulated refunds" /></div>
...
export function SupportWorkspace({ consoleType }: { consoleType?: "admin" | "superadmin" }) {
  ...
  const content = <>
    ...
    <div className="metric-grid four live-metrics"><Metric label="Total Cases" value={String(cases.length)} note="Customer support cases" /><Metric label="Open Cases" value={String(cases.filter((c) => c.status !== "resolved" && c.status !== "closed").length)} note="Requires agent response" /><Metric label="High Priority" value={String(cases.filter((c) => c.priority === "high" || c.priority === "urgent").length)} note="Escalated tickets" /><Metric label="Total Messages" value={String(messages.length)} note="Thread messages" /></div>
...
export function ComplianceConsole() {
  ...
  return <ConsoleShell consoleType="superadmin"><PageTitle eyebrow="GOVERNANCE / COMPLIANCE" title="Compliance review" copy="Upload protected evidence, track expiry, and record permission-checked reviews." />
    <div className="metric-grid four live-metrics"><Metric label="Compliance Files" value={String(documents.length)} note="Uploaded documents" /><Metric label="Approved" value={String(documents.filter((d) => d.review_status === "approved").length)} note="Verified compliance records" /><Metric label="Under Review" value={String(documents.filter((d) => d.review_status === "pending" || d.review_status === "review").length)} note="Awaiting check" /><Metric label="Organizations" value={String(organizations.length)} note="Monitored operators" /></div>
...
export function SettingsConsole({ consoleType }: { consoleType: "admin" | "superadmin" }) {
  ...
  return <ConsoleShell consoleType={consoleType}><PageTitle eyebrow={consoleType === "admin" ? "WORKSPACE / SETTINGS" : "PLATFORM / CONFIGURATION"} title={consoleType === "admin" ? "Branch settings" : "Global configuration"} copy="Create and update versioned JSON settings through an optimistic-concurrency database operation." />
    <div className="metric-grid four live-metrics"><Metric label="Configured Settings" value={String(rows.length)} note="Versioned setting keys" /><Metric label="Branches" value={String(branches.length)} note="Branch operational scopes" /><Metric label="Scope" value={consoleType === "admin" ? "Branch Settings" : "Global Platform"} note="Permission scope" /><Metric label="Latest Version" value={String(rows[0]?.version ?? 1)} note="Optimistic concurrency" /></div>
...
export function AccessInvitationsConsole() {
  ...
  return <ConsoleShell consoleType="superadmin"><PageTitle eyebrow="SECURITY / INVITATIONS" title="Access invitations" copy="Create scoped, expiring invitation records and revoke them. Email delivery remains disabled until a provider is configured." />
    <div className="metric-grid four live-metrics"><Metric label="Total Invitations" value={String(rows.length)} note="Access invitations" /><Metric label="Pending Active" value={String(rows.filter((i) => invitationStatus(i) === "pending").length)} note="Valid pending invites" /><Metric label="Accepted" value={String(rows.filter((i) => invitationStatus(i) === "accepted").length)} note="Claimed accounts" /><Metric label="Organizations" value={String(organizations.length)} note="Target organizations" /></div>
...
export function SettlementConsole() {
  ...
  return <ConsoleShell consoleType="superadmin"><PageTitle eyebrow="PLATFORM / MOCK FINANCE" title="Settlement governance" copy="Review and advance mock settlement records. A paid state is a simulation only and never sends a real payout." />
    <div className="metric-grid four live-metrics"><Metric label="Total Settlements" value={String(rows.length)} note="Mock payout batches" /><Metric label="Organizations" value={String(organizations.length)} note="Governed tenant orgs" /><Metric label="Pending Review" value={String(rows.filter((s) => s.status === "draft" || s.status === "review").length)} note="Awaiting approval" /><Metric label="Simulated Payouts" value={`₱${rows.reduce((a, b) => a + Number(b.payout_amount), 0).toLocaleString("en-PH")}`} note="Mock financial total" /></div>
